...
namespace QuickLZ
{
    ...
    static inline u32 hash_func(u32 i)
    {
#if QLZ_COMPRESSION_LEVEL == 2
    ...
    static inline u32 fast_read(void const *src, u32 bytes)
    {
#ifndef X86X64
    ...
    static inline u32 hashat(const unsigned char *src)
    {
        u32 fetch, hash;
    ...
    static inline void fast_write(u32 f, void *dst, size_t bytes)
    {
#ifndef X86X64
    ...
    static inline void memcpy_up(unsigned char *dst, const unsigned char *src, u32 n)
    {
    ...
    static inline void update_hash(qlz_state_decompress *state, const unsigned char *s)
    {
#if QLZ_COMPRESSION_LEVEL == 1
        u32 hash;
        hash = hashat(s);
    ...
    static size_t qlz_compress_core(const unsigned char *source, unsigned char *destination, size_t size, qlz_state_compress *state)
    {
        ...
        while(src <= last_matchstart)
        {
...
#if QLZ_COMPRESSION_LEVEL == 1
            {
                ...
                if (cached == 0 && o != OFFSET_BASE && (src - o > MINOFFSET || (src == o + 1 && lits >= 3 && src > source + 3 && same(src - 3, 6))))
                {
                    ...
                    else
                    {
                        ...
                        if (matchlen < 18)
                        {
                            fast_write((u32)(matchlen - 2) | hash, dst, 2);
                            dst += 2;
                        }
                        else
                        {
                            fast_write((u32)(matchlen << 16) | hash, dst, 3);
                            dst += 3;
                        }
                    }
                    fetch = fast_read(src, 3);
...
#elif QLZ_COMPRESSION_LEVEL >= 2
            {
                ...
                u32 hash, matchlen, k, m, best_k = 0;
                unsigned char c;
                ...
                if(matchlen > 2 && src - o < 131071)
                {
                    ...
                    {
                        u32 f = (u32)((offset << 2) | 1);
                        fast_write(f, dst, 2);
                        dst += 2;
                    }
                    else if (matchlen <= 18 && offset <= 1023)
                    {
                        u32 f = ((matchlen - 3) << 2) | ((u32)offset << 6) | 2;
                        fast_write(f, dst, 2);
                        dst += 2;
                    }
                    ...
                    {
                        u32 f = ((matchlen - 2) << 2) | ((u32)offset << 7) | 3;
                        fast_write(f, dst, 3);
                        dst += 3;
                    }
                    else
                    {
                        u32 f = ((matchlen - 3) << 7) | ((u32)offset << 15) | 3;
                        fast_write(f, dst, 4);
                        dst += 4;
                    }
                }
                ...
                if(matchlen > 2)
                {
                    ...
                    if (matchlen < 10)
                    {
                        u32 f = best_k | ((matchlen - 2) << 2) | (hash << 5);
                        fast_write(f, dst, 2);
                        dst += 2;
                    }
                    else
                    {
                        u32 f = best_k | (matchlen << 16) | (hash << 5);
                        fast_write(f, dst, 3);
                        dst += 3;
                    }
                }
    ...
    static size_t qlz_decompress_core(const unsigned char *source, unsigned char *destination, size_t size, qlz_state_decompress *state, const unsigned char *history)
    {
        ...
        static const u32 bitlut[16] = {4, 0, 1, 0, 2, 0, 1, 0, 3, 0, 1, 0, 2, 0, 1, 0};
        ...
        for(;;)
        {
            ...
            if ((cword_val & 1) == 1)
            {
                ...
                if(matchlen > (u32)(last_destination_byte - dst - UNCOMPRESSED_END + 1))
                    return 0;
#endif
    ...
    size_t qlz_compress(const void *source, char *destination, size_t size, qlz_state_compress *state)
    {
        ...
        else
        {
            *destination = (unsigned char)(2 | compressed);
            fast_write((u32)r, destination + 1, 4);
            fast_write((u32)size, destination + 5, 4);
        }
